...
$(function() {
    ...
    $('div[class*=edit-]').on('click', function(e) {  
        if($(this).hasClass("edit-vision")) 
            $('#formV').css({"display": "" }) && $('#vs').hide().css({"display": "none"});
        if($(this).hasClass("edit-mission")) 
             $('#formM').css({"display": "" }) && $('#ms').hide().css({"display": "none"});
        if($(this).hasClass("edit-obj")) 
            $('#formO').css({"display": "" }) && $('#os').hide().css({"display": "none"});
    })

    

    $.get('./assets/hndlr/aliases.php',
        function(data) {
            let results = []
            let id = JSON.parse(data)
            id.forEach(el => {
                el.alias == 'vision' ? $('textarea[id=vision_body]').text(el.content) && $('#vision-view').html(el.content)  :
                el.alias == 'mission' ? $('textarea[id=mission_body]').text(el.content) && $('#mission-view').html(el.content) :
                el.alias == 'objectives' ? $('textarea[id=obj_body]').text(el.content) && $('#obj-view').html(el.content) : '';
            });
        }
    );
